...
window = Tk()
window.title('Bubble Blaster: 30 Level Final Edition')
...
def create_bubble():
    ...
    if chance < 15: 
        bub_id.append(c.create_oval(x-r, y-r, x+r, y+r, outline='red', fill='black', width=2))
        bub_type.append('bomb')
    elif chance < 22: 
        bub_id.append(c.create_oval(x-r, y-r, x+r, y+r, outline='lightgreen', fill='green', width=2))
        bub_type.append('bonus')
    elif chance < 28: 
        bub_id.append(c.create_oval(x-r, y-r, x+r, y+r, outline='cyan', fill='blue', width=2))
        bub_type.append('shield')
    else: 
        bub_id.append(c.create_oval(x-r, y-r, x+r, y+r, outline='white'))
        bub_type.append('normal')
...
def main_game_loop():
    global end_time, score, level, lives, shield_active, shield_ui
        ...
        if time() < end_time and lives > 0:
            ...
            for i in range(len(bub_id)-1, -1, -1):
                ...
                if dist < (15 + bub_r[i]):
                    ...
                    if t == 'bomb':
                        if shield_active: 
                            shield_active = False; c.delete(shield_ui)
                        else: 
                            lives -= 1; play_sound(BOMB_SOUND)
                    elif t == 'bonus': 
                        end_time += 5; play_sound(POP_SOUND)
                    elif t == 'shield':
                        ...
                        score += bub_r[i]; play_sound(POP_SOUND)
...
def start_game():
    global score, level, lives, end_time, shield_active; c.delete('all'); setup_ui()
    score, level, lives, shield_active = 0, 1, 3, False
    end_time = time() + 30
